...
func (DB2G db2g) toString(bytes []byte, column *column, conn *DmConnection) string {
	...
	case BIGINT:
		return strconv.FormatInt(Dm_build_650.Dm_build_880(bytes), 10)
	case REAL:
		return strconv.FormatFloat(float64(Dm_build_650.Dm_build_883(bytes)), 'f', -1, 32)
	case DOUBLE:
		return strconv.FormatFloat(Dm_build_650.Dm_build_886(bytes), 'f', -1, 64)
	case DECIMAL:
...
func (DB2G db2g) toInt64(bytes []byte, column *column, conn *DmConnection) (int64, error) {
	...
	case BIGINT:
		return Dm_build_650.Dm_build_880(bytes), nil
	case REAL:
